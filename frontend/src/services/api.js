...
const resolveApiBase = () => {
  ...
  if (typeof window !== 'undefined') {
    ...
    // Si abres el frontend directo en :3001, el gateway vive en :8080.
...
api.interceptors.response.use(
  ...
  err => {
    ...
    const validation = Array.isArray(data?.errors) ? data.errors.map((e) => e.msg || e.message).filter(Boolean).join(', ') : '';
